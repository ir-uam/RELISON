 ...
 */
package es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.swap.graph.comms.gini.edge.sizenorm;
...
import es.uam.eps.ir.sonalire.links.recommendation.reranking.global.swap.comm.gini.edge.sizenormalized.OuterSizeNormalizedInterCommunityEdgeGiniComplement;
...
import static es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.RerankerIdentifiers.OUTERSNICEDGEGINI;


/**
 * Grid search for a reranker that reduces the Gini index of the number of links between pairs of communities (restricted
 * to nodes between communities). It also promotes links outside communities.
 ...
 *
 */
public class SizeNormInterCommunityOuterEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    /**
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid, int cutoff, Supplier<Normalizer<U>> norm, Graph<U> graph, Communities<U> comms)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(OUTERSNICEDGEGINI + "-" + lambda, () -> new OuterSizeNormalizedInterCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms)
        ));
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(OUTERSNICEDGEGINI + "-" + lambda, (cutoff, norm, graph, comms) ->
                new OuterSizeNormalizedInterCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms)
        ));

 ...
 */
package es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.swap.graph.comms.gini.degree.sizenorm;
...
import es.uam.eps.ir.sonalire.links.recommendation.reranking.global.swap.comm.gini.degree.sizenormalized.SizeNormalizedCompleteCommunityDegreeGiniComplement;
...
import static es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.RerankerIdentifiers.SNCDEGREEGINI;
 ...
 * 
 */
public class SizeNormCompleteCommunityDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    /**
            ...
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                    rerankers.put(SNCDEGREEGINI + "-" + orient + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                        new SizeNormalizedCompleteCommunityDegreeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop, orient)
                    )
                )
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                    rerankers.put(SNCDEGREEGINI + "-" + orient + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, (cutoff, norm, graph, comms) ->
                        new SizeNormalizedCompleteCommunityDegreeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop, orient)
                    )
                )
            )

...
import es.uam.eps.ir.sonalire.links.recommendation.reranking.global.swap.comm.gini.edge.sizenormalized.SizeNormalizedSemiCompleteCommunityEdgeGiniComplement;
...
import static es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.RerankerIdentifiers.SNSCEDGEGINI;


/**
 * Grid search for a reranker that reduces the Gini index of the number of links between pairs of communities.
 * All links within communities are included in the same category.
 ...
 *
 */
public class SizeNormSemiCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    /**
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SNSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new SizeNormalizedSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop)
                )
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SNSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new SizeNormalizedSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop)
                )
            )
        );

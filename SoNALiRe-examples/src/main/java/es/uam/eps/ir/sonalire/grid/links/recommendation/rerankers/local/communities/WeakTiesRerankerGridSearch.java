 ...
 */
package es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.local.communities;
...
import es.uam.eps.ir.sonalire.links.recommendation.reranking.global.local.communities.WeakTiesReranker;
...
import static es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.RerankerIdentifiers.WEAKTIES;
...
public class WeakTiesRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid, int cutoff, Supplier<Normalizer<U>> norm, Graph<U> graph, Communities<U> comms)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(WEAKTIES + "-" + lambda, () -> new WeakTiesReranker<>(lambda, cutoff, norm, graph, comms)));
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(WEAKTIES + "-" + lambda, (cutoff, norm, graph, comms) -> new WeakTiesReranker<>(lambda, cutoff, norm, graph, comms)));

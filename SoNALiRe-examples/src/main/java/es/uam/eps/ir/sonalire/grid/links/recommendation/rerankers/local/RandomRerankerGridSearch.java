 ...
 */
package es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.local;
...
import es.uam.eps.ir.sonalire.links.recommendation.reranking.global.local.LocalRandomReranker;
...
import static es.uam.eps.ir.sonalire.grid.links.recommendation.rerankers.RerankerIdentifiers.RANDOM;
...
public class RandomRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid, int cutoff, Supplier<Normalizer<U>> norm, Graph<U> graph, Communities<U> comms)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(RANDOM + "-" + lambda, () -> new LocalRandomReranker<>(cutoff, lambda, norm)));
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(RANDOM + "-" + lambda, (cutoff, norm, graph, comms) -> new LocalRandomReranker<>(cutoff, lambda, norm)));
        return rerankers;
    }
}

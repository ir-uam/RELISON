...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.InterCommunityEdgeGiniComplement;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(ICEDGEGINI + "-" + lambda, () ->
                new InterCommunityEdgeGiniComplement<>(lambda, cutoff, norm, rank, graph, comms)
            )
        );

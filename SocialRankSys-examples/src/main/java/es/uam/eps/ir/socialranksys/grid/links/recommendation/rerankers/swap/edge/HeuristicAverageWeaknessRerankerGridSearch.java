...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.edge.HeuristicWeaknessReranker;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.HEURISTICAVGWEAKNESS;
...
public class HeuristicAverageWeaknessRerankerGridSearch<U> implements RerankerGridSearch<U>
{
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getIntegerValues(MODE).forEach(mode ->
                rerankers.put(HEURISTICAVGWEAKNESS + "-" + mode + "-" + lambda, () ->
                    new HeuristicWeaknessReranker<>(lambda, cutoff, norm, graph, mode)
                )
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getIntegerValues(MODE).forEach(mode ->
                rerankers.put(HEURISTICAVGWEAKNESS + "-" + mode + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new HeuristicWeaknessReranker<>(lambda, cutoff, norm, graph, mode)
                )
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.InterCommunityOuterEdgeGiniReranker;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(OUTERICEDGEGINI + "-" + lambda, () ->
                new InterCommunityOuterEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms)
            )
        );

...
public class SizeNormSemiCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
     ...
     */
    public SizeNormSemiCompleteCommunityEdgeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SNSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new SizeNormalizedSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, rank, graph, comms, autoloop)
                )
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.AlternativeSemiCompleteCommunityEdgeGiniComplement;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.ALTSCEDGEGINI;


/**
 * Grid search for a reranker that reduces the Gini index of the number of links between pairs of communities.
 * This reranker takes all the intra-community links in the same group.
 *
...
public class AlternativeSemiCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(ALTSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new AlternativeSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(ALTSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new AlternativeSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );

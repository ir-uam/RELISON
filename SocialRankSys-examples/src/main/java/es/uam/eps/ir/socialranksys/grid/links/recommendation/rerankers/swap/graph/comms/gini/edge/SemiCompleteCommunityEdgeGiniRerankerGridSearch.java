...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.SemiCompleteCommunityEdgeGiniReranker;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new SemiCompleteCommunityEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop))
            )
        );

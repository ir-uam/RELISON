...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.SemiCompleteCommunityEdgeGiniComplement;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.SCEDGEGINI;
...
public class SemiCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new SemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(SCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new SemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );

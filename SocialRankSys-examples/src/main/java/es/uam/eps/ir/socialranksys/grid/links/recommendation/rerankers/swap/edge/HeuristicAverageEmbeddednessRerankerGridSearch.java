...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.edge.HeuristicEmbedednessReranker;

import java.util.HashMap;
import java.util.Map;
import java.util.function.Supplier;

import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.HEURISTICAVGEMBEDDEDNESS;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getIntegerValues(MODE).forEach(mode ->
                rerankers.put(HEURISTICAVGEMBEDDEDNESS + "-" + mode + "-" + lambda, () ->
                    new HeuristicEmbedednessReranker<>(lambda, cutoff, norm, rank, graph, mode)
                )
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.OuterAlternativeSemiCompleteCommunityEdgeGiniComplement;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.ALTOUTERSCEDGEGINI;

/**
 * Grid search for a reranker that reduces the Gini index of the number of links between pairs of communities.
 * This reranker takes all the intra-community links in the same group. It forces links to go outside
 * communities.
 *
...
public class AlternativeSemiCompleteCommunityOuterEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(ALTOUTERSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new OuterAlternativeSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(ALTOUTERSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new OuterAlternativeSemiCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, graph, comms, autoloop))
            )
        );

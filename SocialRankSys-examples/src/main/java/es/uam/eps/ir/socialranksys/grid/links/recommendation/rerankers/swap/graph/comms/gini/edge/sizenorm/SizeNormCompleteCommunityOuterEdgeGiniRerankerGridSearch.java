...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.sizenormalized.SizeNormalizedCompleteCommunityOuterEdgeGiniReranker;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(OUTERSNCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new SizeNormalizedCompleteCommunityOuterEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop)
                )
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.graph.ClusteringCoefficientComplementReranker;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.CLUSTCOEFCOMPL;
...
public class ClusteringCoefficientComplementRerankerGridSearch<U> implements RerankerGridSearch<U>
{
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid, int cutoff, Supplier<Normalizer<U>> norm, Graph<U> graph, Communities<U> comms)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(CLUSTCOEFCOMPL + "-" + lambda, () ->
                new ClusteringCoefficientComplementReranker<>(lambda, cutoff, norm, graph))
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(CLUSTCOEFCOMPL + "-" + lambda, (cutoff, norm, graph, comms) ->
                new ClusteringCoefficientComplementReranker<>(lambda, cutoff, norm, graph))
        );

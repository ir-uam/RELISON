...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.degree.CompleteCommunityDegreeGiniComplement;

import java.util.HashMap;
import java.util.Map;
import java.util.function.Supplier;

import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.CDEGREEGINI;
            ...
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                    rerankers.put(CDEGREEGINI + "-" + orient + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                        new CompleteCommunityDegreeGiniComplement<>(lambda, cutoff, norm, rank, graph, comms, autoloop, orient))
                )
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.degree.CompleteCommunityDegreeGiniReranker;
            ...
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                    rerankers.put(CDEGREEGINI + "-" + orient + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                        new CompleteCommunityDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop, orient))
                )
            )
        );

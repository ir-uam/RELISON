...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.OuterCompleteCommunityEdgeGiniComplement;

import java.util.HashMap;
import java.util.Map;
import java.util.function.Supplier;

import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.OUTERCEDGEGINI;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(OUTERCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new OuterCompleteCommunityEdgeGiniComplement<>(lambda, cutoff, norm, rank, graph, comms, autoloop))
            )
        );

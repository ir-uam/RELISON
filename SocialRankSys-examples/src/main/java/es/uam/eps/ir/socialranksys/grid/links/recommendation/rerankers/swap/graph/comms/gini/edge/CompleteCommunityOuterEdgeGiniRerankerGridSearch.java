...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.edge.CompleteCommunityOuterEdgeGiniReranker;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
                rerankers.put(OUTERCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () ->
                    new CompleteCommunityOuterEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop))
            )
        );

...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.graph.ClusteringCoefficientReranker;

import java.util.HashMap;
import java.util.Map;
import java.util.function.Supplier;

import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.CLUSTCOEF;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            rerankers.put(CLUSTCOEF + "-" + lambda, () ->
                new ClusteringCoefficientReranker<>(lambda, cutoff, norm, rank, graph))
        );

 ...
 */
package es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.swap.graph.comms.gini.degree;
...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.degree.InterCommunityDegreeGiniComplement;
...
import static es.uam.eps.ir.socialranksys.grid.links.recommendation.rerankers.RerankerIdentifiers.ICDEGREEGINI;
...
public class InterCommunityDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U>
{
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                rerankers.put(ICDEGREEGINI + "-" + orient + "-" + lambda, () ->
                    new InterCommunityDegreeGiniComplement<>(lambda, cutoff, norm, graph, comms, orient))
            )
        );
    ...
    public Map<String, GlobalRerankerFunction<U>> grid(Grid grid)
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                rerankers.put(ICDEGREEGINI + "-" + orient + "-" + lambda, (cutoff, norm, graph, comms) ->
                    new InterCommunityDegreeGiniComplement<>(lambda, cutoff, norm, graph, comms, orient))
            )
        );

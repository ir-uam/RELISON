...
import es.uam.eps.ir.socialranksys.links.recommendation.reranking.global.swap.comm.gini.degree.InterCommunityDegreeGiniReranker;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda ->
            grid.getOrientationValues(ORIENTATION).forEach(orient ->
                rerankers.put(ICDEGREEGINI + "-" + orient + "-" + lambda, () ->
                    new InterCommunityDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, orient))
            )
        );

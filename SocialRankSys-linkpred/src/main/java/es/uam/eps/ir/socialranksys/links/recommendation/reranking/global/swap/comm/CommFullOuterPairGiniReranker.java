...
public class CommFullOuterPairGiniReranker<U> extends CommunityReranker<U> 
{
    ...
    protected double novAddDelete(U u, Tuple2od<U> itemValue, Tuple2od<U> compared)
    {
        ...
        {
            double toSum;
            if((userComm == recommComm && userComm == delComm) || (!(userComm == recommComm) && !(userComm == delComm)))
            {
                toSum = 0.0;
            }
            ...
            {
                if(index == newIdx && !(userComm == recommComm))
                {
                    atom.addAndGet(1.0);
            ...
            return this.numWeak + toSum + 1.0 - gini.compute(stream.boxed(), true, communities.getNumCommunities()*(communities.getNumCommunities()-1), atom.doubleValue());
        }
        else
        {
            int newIdx = 0;

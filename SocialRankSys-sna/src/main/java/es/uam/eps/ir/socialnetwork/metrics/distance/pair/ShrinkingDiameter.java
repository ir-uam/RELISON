 ...
 */
package es.uam.eps.ir.socialnetwork.metrics.distance.pair;
...
public class ShrinkingDiameter<U> extends AbstractPairMetric<U>
{
    ...
    public double compute(Graph<U> graph, U orig, U dest)
    {
        ...
        distances.keySet().forEach((u) ->
            distances.get(u).keySet().forEach((v) ->
            {
    ...
    private double computePair(Graph<U> graph, U u, U v, Set<U> uCIn, Set<U> uCOut, Set<U> vCIn, Set<U> vCOut, TreeMap<Double, List<Pair<U>>> pairsAtDistance, Map<U,Map<U,Double>> distances) 
    {
        ...
        if(graph.containsEdge(u,v)) return 0.0;
        ...
        if(!Double.isFinite(distances.get(u).get(v)))
        {
            ...
            if(auxdiam >= diam) return diam - auxdiam; // else: for this part, this is not increasing
        }
        ...
        for(double key : pairsAtDistance.descendingKeySet())
        {
            ...
            for(Pair<U> pairs : pairsAtDistance.get(key))
            {
                ...
                if(graph.isDirected()) // First case: the graph is directed.
                {
                    ...
                    if(w2.equals(u) || w1.equals(v)) return diam - key;
                    else if(w1.equals(u) && w2.equals(v)) auxDistances.add(1.0); // if the pair corresponds to the edge we want to add: distance == 1.0
                    else if(w1.equals(u)) // Origin node: u
                    {
                        // If we cannot reach the node from v, it is the same.
                        if(!vCOut.contains(w2)) return diam - key;
                        else // Update:
                        {
                            double dist = Math.min(key, 1 + distances.get(v).get(w2));
                            // If this happens: the diameter does not reduce -> therefore, we found it.
                            if(dist == key) return diam - key;
                            else auxDistances.add(dist);
                        }
                    }
                    else if(w2.equals(v))  // Destination node: v
                    {
                        ...
                        if(!uCIn.contains(w1)) return diam - key;
                        else // Update:
                        {
                    ...
                    else // !w1.equals(u) && !w2.equals(v)
                    {
                        ...
                        if(!uCIn.contains(w1) || !vCOut.contains(w2)) return diam - key;
                        else
                        {
                ...
                else // if is undirected
                {
                    ...
                    else if(w2.equals(u))
                    {
                        ...
                        if(!vCOut.contains(w1)) return diam - key;
                        else
                        {
                    ...
                    else if(w1.equals(v))
                    {
                        ...
                        if(!uCIn.contains(w2)) return diam - key;
                        else
                        {
